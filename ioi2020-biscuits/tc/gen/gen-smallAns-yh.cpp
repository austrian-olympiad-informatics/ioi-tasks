...
int main(int argc, char* argv[]) {
    ...
    while(q--) {
        ...
        do {
            ...
            a[0] = x + rnd.next(20);
            for(int i=bits; i<__builtin_clzll(x)-4; i++) {
                a[i] = (0.48+rnd.next(0.025))*x;
            }
            while(count_tastiness(x,a)>=count_max || sum(a)>tastiness_max) {
                int idx = rnd.next(1, bits-1);
                a[idx] *= rnd.next(0.99, 0.995);
            }
        } while(nontrivial<min(min(bits*3/4,bits/2+9),34));
        /*while(count_tastiness(x, a)<count_max) {
            if(i_max<3) break;
            incr = rnd.next(i_max);
            a[incr]++;
            if(sum(a)>= tastiness_max) {
                a[incr]--;
                i_max--;
            }
        }
        if(count_tastiness(x,a)>=count_max) a[incr]--;*/
