...
int construct_roads(std::vector<int> x, std::vector<int> y) {
  ...
  std::vector<std::tuple<int, int, int, int, int>> pathways;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (it != fountains[x[i] + 2].end() && it->first == y[i]) {
      ...
      pathways.emplace_back(x[i] + 1, ny, x[i] + y[i], i, it->second);
    ...
    if (it != fountains[x[i]].end() && it->first == y[i] + 2) {
      ...
      pathways.emplace_back(nx, y[i] + 1, x[i] + y[i], i, it->second);
      adj[i].emplace_back(it->second);
      adj[it->second].emplace_back(i);
    }
  }
  ...
  for (int i = 0; i < static_cast<int>(pathways.size()); ++i) {
    ...
    u.emplace_back(std::get<3>(pathways[i]));
    v.emplace_back(std::get<4>(pathways[i]));
